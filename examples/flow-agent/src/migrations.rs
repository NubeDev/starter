...
//! `starter-store-postgres` (DOCS/agent/MEMORY.md Phase M-A) so the
...
use starter_store_postgres::flow::AGENT_SESSION_MIGRATION_SOURCE;
...
/// dep direction (starter-prefs → starter-store-postgres); consumers
/// reconstruct it from the public `PG_MIGRATIONS` static. See
/// `starter_prefs::store::PG_MIGRATIONS` for the documented pattern.
