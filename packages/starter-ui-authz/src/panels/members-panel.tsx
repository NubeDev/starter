...
export function MembersPanel({
  ...
  if (!tenantId) {
    return (
      <section className="grid gap-6">
        <header>
          <h2 className="text-xl font-semibold tracking-tight">{m.members.title}</h2>
          <p className="text-sm text-[color:var(--color-subtle,#6b7280)]">{m.members.description}</p>
        </header>
  ...
  return (
    <section className="grid gap-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{m.members.form.submit}</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={onAdd} className="grid grid-cols-1 gap-3 sm:grid-cols-[2fr_1fr_auto] sm:items-end">
            ...
            <Button type="submit" disabled={add.isPending}>{m.members.form.submit}</Button>
          </form>
          {add.error ? <p className="mt-2 text-xs text-[color:var(--color-danger,#dc2626)]">{add.error.message}</p> : null}
        </CardContent>
      </Card>
        ...
        <DataTable
          ...
          rows={members.map((mem) => (
            <tr key={`${mem.tenant_id}:${mem.user_id}`}>
              <Td><code className="text-xs">{mem.user_id}</code></Td>
              <Td>
                <Select
                  value={mem.role}
                  onValueChange={(v) => {
