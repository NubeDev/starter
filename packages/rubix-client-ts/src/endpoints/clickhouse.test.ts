// Wire-contract tests for the `rubix.clickhouse.*` endpoint methods.
...
describe("RubixClient clickhouse endpoints", () => {
  it("ruleWrite() POSTs to /api/v1/tools/rubix.clickhouse.rule.write with CSRF header", async () => {
    const body = {
    ...
    expect(call.url).toBe("http://t/api/v1/tools/rubix.clickhouse.rule.write");
  ...
  it("martCreate() POSTs to /api/v1/tools/rubix.clickhouse.mart.create with CSRF header", async () => {
    const body = {
    ...
    expect(calls[0]!.url).toBe("http://t/api/v1/tools/rubix.clickhouse.mart.create");
  ...
  it("retentionSet() POSTs to /api/v1/tools/rubix.clickhouse.retention.set with CSRF header", async () => {
    const body = {
    ...
    expect(calls[0]!.method).toBe("POST");
    expect(calls[0]!.url).toBe("http://t/api/v1/tools/rubix.clickhouse.retention.set");
    expect(calls[0]!.headers["X-CSRF-Token"]).toBe(CSRF);
