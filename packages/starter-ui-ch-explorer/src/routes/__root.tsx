...
function Shutdown() {
  // Removed in the rubix fork — server lifecycle is owned by
  // `starter-server`, not the explorer UI. Stub kept (and exported
  // for the lint-only side effect) so a future merge from upstream
  // surfaces the diff rather than silently re-adding the button.
  return null;
}
void Shutdown;
...
function Github({ className }: { className: string }) {
  // Unused in the rubix fork (the upstream sql-studio GitHub link in
  // the header is dropped). Kept so merges from upstream apply
  // cleanly. The `void` reference below silences
  // `noUnusedLocals`.
  void className;
  return null;
}
