//! [`PgSetupRunStore`] — the thin run index over flow `RunId`s
//! (DOCS §4/§8). Postgres twin of `SqliteSetupRunStore`.

use async_trait::async_trait;
use sqlx::Row;
use starter_flow_spi::flow::RunId;
use starter_setup_spi::error::{SetupError, SetupResult};
use starter_setup_spi::model::{Progress, SemVer, SetupRun, SetupRunStatus, TemplateId};
use starter_setup_spi::store::{SetupRunFilter, SetupRunStore};

use super::{fmt_ts, parse_ts};
use crate::pool::Pool;

/// Postgres-backed [`SetupRunStore`].
#[derive(Clone)]
pub struct PgSetupRunStore {
    pool: Pool,
}

impl PgSetupRunStore {
...
fn row_to_run(row: &sqlx::postgres::PgRow) -> SetupResult<SetupRun> {
    ...
    let progress_v: serde_json::Value = row.try_get("progress_json").map_err(backend)?;
    let failed_node: Option<String> = row.try_get("failed_node").map_err(backend)?;
    let resumable: bool = row.try_get("resumable").map_err(backend)?;
    ...
    let finished_at: Option<chrono::DateTime<chrono::Utc>> =
        row.try_get("finished_at").map_err(backend)?;
    ...
    Ok(SetupRun {
        ...
        progress: serde_json::from_value(progress_v)
            .map_err(|e| SetupError::Backend(format!("progress: {e}")))?,
        failed_node,
        resumable,
        created_at: fmt_ts(created_at),
        finished_at: finished_at.map(fmt_ts),
    })
}

#[async_trait]
impl SetupRunStore for PgSetupRunStore {
    async fn record(&self, run: SetupRun) -> SetupResult<()> {
        let pool = self.pool.sqlx();
        let progress = serde_json::to_value(&run.progress)
            .map_err(|e| SetupError::Backend(format!("progress: {e}")))?;
        let created = parse_ts(&run.created_at).map_err(SetupError::Backend)?;
        let finished = run
            .finished_at
            .as_deref()
            .map(parse_ts)
            .transpose()
            .map_err(SetupError::Backend)?;
        sqlx::query(
    ...
    async fn list(&self, filter: SetupRunFilter) -> SetupResult<Vec<SetupRun>> {
        let pool = self.pool.sqlx();
        let mut sql = String::from("SELECT * FROM setup_runs WHERE TRUE");
        let mut n = 0;
        if filter.owner.is_some() {
            n += 1;
            sql.push_str(&format!(" AND owner = ${n}"));
        }
        ...
        if filter.template_id.is_some() {
            n += 1;
            sql.push_str(&format!(" AND template_id = ${n}"));
        }
    ...
    async fn update_progress(
        ...
        let pool = self.pool.sqlx();
        let progress_v = serde_json::to_value(&progress)
            .map_err(|e| SetupError::Backend(format!("progress: {e}")))?;
    ...
    async fn mark_failed(
        ...
        sqlx::query(
            "UPDATE setup_runs SET status = 'Failed', failed_node = $1, resumable = $2, \
             finished_at = COALESCE(finished_at, NOW()) WHERE run_id = $3",
        )
        .bind(failed_node)
        .bind(resumable)
        .bind(run_id.0.to_string())
    ...
    async fn mark_finished(
        ...
        let pool = self.pool.sqlx();
        let finished = parse_ts(&finished_at).map_err(SetupError::Backend)?;
        ...
        let rows = sqlx::query(
            "SELECT run_id FROM setup_runs \
             WHERE status IN ('Pending', 'Running') \
                OR (status = 'Failed' AND resumable = TRUE)",
        )
        .fetch_all(pool)
        .await
        .map_err(backend)?;
