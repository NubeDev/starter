//! [`PgFlowStore`] — flow definitions, immutable revisions,
//! per-flow head pointer (D-F3.3). Postgres twin of
//! `SqliteFlowStore`.
...
use super::schema::{from_value, sqlx_backend, sqlx_to_flow};
use crate::pool::Pool;

/// Postgres-backed [`FlowStore`].
///
...
impl PgFlowStore {
    /// Construct a [`PgFlowStore`] over an existing [`Pool`].
...
impl FlowStore for PgFlowStore {
    async fn load(
        ...
        let body = from_value::<serde_json::Value>("flow_revisions.body_json", row.0)?;
    ...
    async fn put(&self, revision: FlowRevision) -> FlowResult<FlowRevisionId> {
        let pool = self.pool.sqlx();
        let body_value = revision.body.clone();
        ...
        // `ON CONFLICT … DO NOTHING` so re-puts of the same
        // (flow_id, revision_id) pair are idempotent — revisions
        // are immutable per SCOPE "Decisions made"; same rationale
        // as the SQLite twin's `INSERT OR IGNORE`.
        sqlx::query(
            "INSERT INTO flow_revisions \
             (flow_id, revision_id, body_json, source) VALUES ($1, $2, $3, $4) \
             ON CONFLICT (flow_id, revision_id) DO NOTHING",
        )
        .bind(&flow_id_s)
        .bind(&revision_id_s)
        .bind(&body_value)
