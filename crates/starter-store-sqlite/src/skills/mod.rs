//! SQLite implementation of the `starter-skills` [`ApprovalStore`]
//! trait (Phase 5, R-skills-7).
//!
//! Gated behind the default-on `skill-approvals` cargo feature so
//! consumers who do not need skill approvals can opt out with
//! `--no-default-features` and pay neither the compile cost nor the
//! `starter-skills` dep. Migrations live under `migrations/skills/`
//! and are exposed as [`SKILL_APPROVALS_MIGRATOR`] /
//! [`SKILL_APPROVALS_MIGRATION_SOURCE`] for the namespaced runner —
//! the same shape `FLOW_MIGRATION_SOURCE` uses.
//!
