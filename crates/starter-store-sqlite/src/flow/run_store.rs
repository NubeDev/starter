//! [`SqliteRunStore`] — run lifecycle, per-tick checkpoints,
//! dedup lookup (D-F3.3, D-F3.8, D-F3.9, D-F3.12).
...
impl SqliteRunStore {
    /// Construct a [`SqliteRunStore`] over an existing [`Pool`].
...
/// Serialize the slot-write batch for the checkpoint row. Kept
/// out of band of `schema::to_json` so the borrow stays explicit
/// (the SPI passes the batch by slice, not by owned `Vec`).
fn writes_to_json(writes: &[(SlotRef, SlotValue)]) -> FlowResult<String> {
    serde_json::to_string(writes).map_err(|e| FlowError::Backend(format!("serialize writes: {e}")))
}
...
impl RunStore for SqliteRunStore {
    async fn start(
        ...
        let opts_json = to_json(&opts)?;
        let principal_json = to_json(&principal)?;
    ...
    async fn checkpoint(
        ...
        let pool = self.pool.sqlx();
        let writes_json = writes_to_json(writes)?;
        let state_json = to_json(&state)?;
        let run_id_s = run_id.0.to_string();
        // SQLite `INTEGER` is i64; the propagator's tick counter
        // is u64. The 2^63 ceiling is a non-issue (one tick per
        // ns for 292 years), but cast explicitly so the lossy
        // path is visible.
        ...
        // intermediate. sqlx's default `pool.begin()` opens a
        // `BEGIN DEFERRED`; SQLite upgrades to a RESERVED lock on
    ...
    async fn load(&self, run_id: RunId) -> FlowResult<Option<RunCheckpoint>> {
        ...
        let state_json: String = row.try_get("run_state_json").map_err(sqlx_backend)?;
        let writes_json: String = row.try_get("slot_writes_json").map_err(sqlx_backend)?;
        let state: RunState = from_json("run_checkpoints.run_state_json", &state_json)?;
        let writes: Vec<(SlotRef, SlotValue)> =
            from_json("run_checkpoints.slot_writes_json", &writes_json)?;
...
async fn load_retention(pool: &sqlx::SqlitePool, run_id: &str) -> FlowResult<CheckpointRetention> {
    let row: Option<(String,)> = sqlx::query_as("SELECT run_opts_json FROM runs WHERE run_id = ?1")
        .bind(run_id)
        .fetch_optional(pool)
        .await
        .map_err(sqlx_backend)?;
    ...
    let opts: RunOpts = from_json("runs.run_opts_json", &raw)?;
