//! [`ChangeLog`] impl over `starter_changes`.
//!
//! Pagination is keyset on `(at DESC, id DESC)`. The cursor is a
//! base64-encoded `"<at_rfc3339>|<id>"` pair — small, stable, and
//! survives URL/JSON round-trips without escaping.
...
impl ChangeLog for SqliteChangeLog {
    ...
    async fn list(&self, filter: &ChangeFilter) -> Result<ChangePage> {
        ...
        if let Some(v) = &filter.since {
            qb.push(" AND at >= ").push_bind(v.to_rfc3339());
        }
        ...
        let next_cursor = if items.len() > limit as usize {
            let extra = items.pop().expect("len > limit means at least one element");
            // Cursor points at the *last returned* row so the next
            // page's `(at, id) < (...)` strict comparison resumes
            // correctly without skipping or duplicating rows.
            let last = items.last().unwrap_or(&extra);
            Some(encode_cursor(&last.at.to_rfc3339(), &last.id.0))
        } else {
            None
        };
