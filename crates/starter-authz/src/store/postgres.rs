//! Postgres-backed [`PolicyStore`]. Mirror of the sqlite impl
//! with `JSONB` actions and `$N` parameter binding.
...
/// `starter_store_postgres::migrate(&pool).with_source(...)`.
pub static AUTHZ_POSTGRES_MIGRATOR: sqlx::migrate::Migrator =
    sqlx::migrate!("./migrations/starter_authz_postgres");

/// Postgres impl of [`PolicyStore`].
...
fn map_assignment(row: sqlx::postgres::PgRow) -> StoredAssignment {
    StoredAssignment {
        id: row.get(0),
        subject: row.get(1),
        role: row.get(2),
...
fn map_rule(row: sqlx::postgres::PgRow) -> Result<StoredRule, PolicyStoreError> {
    let actions: serde_json::Value = row.get(3);
...
impl PolicyStore for PostgresPolicyStore {
    ...
    async fn list_rules(&self) -> Result<Vec<StoredRule>, PolicyStoreError> {
        let rows = sqlx::query(
            "SELECT id, role, resource, actions, condition, effect, priority, created_by \
    ...
    async fn insert_rule(&self, row: &StoredRule) -> Result<(), PolicyStoreError> {
        let actions_json = serde_json::to_value(&row.actions)
            .map_err(|e| PolicyStoreError::Backend(format!("serialize actions: {e}")))?;
        let res = sqlx::query(
            "INSERT INTO starter_authz_rules \
                (id, role, resource, actions, condition, effect, priority, created_by) \
