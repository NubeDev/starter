//! Shared defaults for AI runs.
//!
//! Held alongside a [`crate::Registry`] at the app-state / flow-context
//! level so call sites never read env vars directly.

use starter_spi::ai::Provider;

/// Per-process AI defaults: which provider to use and which API keys
/// to surface to runners that ask for them.
...
impl AiDefaults {
    /// API key for a given provider, `None` if the provider does not
    /// need one (CLI runners) or it has not been configured.
