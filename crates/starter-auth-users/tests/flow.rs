...
async fn create_admin_then_login_through_session() {
    ...
    let id = create_admin(users.as_ref(), "a@example.com", "hunter22", Role::Admin)
        .await
        .expect("create admin");
    assert!(!id.is_empty());
...
async fn duplicate_admin_email_conflict() {
    ...
    create_admin(users.as_ref(), "dup@example.com", "p1", Role::Admin)
        .await
        .unwrap();
    let err = create_admin(users.as_ref(), "dup@example.com", "p2", Role::Admin)
        .await
        .unwrap_err();
...
async fn revoked_session_no_longer_verifies() {
    ...
    create_admin(users.as_ref(), "r@example.com", "p", Role::Writer)
        .await
        .unwrap();
...
async fn api_token_round_trip() {
    ...
    create_admin(users.as_ref(), "t@example.com", "p", Role::Reader)
        .await
        .unwrap();
...
async fn wrong_secret_after_correct_prefix_is_invalid() {
    let pool = fresh_pool().await;
    let (users, _sessions, tokens) = stores(pool);
    create_admin(users.as_ref(), "w@example.com", "p", Role::Reader)
        .await
        .unwrap();
...
async fn authenticator_dispatches_by_prefix() {
    ...
    create_admin(users.as_ref(), "d@example.com", "p", Role::Admin)
        .await
        .unwrap();
