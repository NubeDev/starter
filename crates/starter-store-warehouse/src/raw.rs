...
impl ChClient {
    ...
    pub async fn fetch_json(&self, sql: &str) -> Result<JsonRows, ChClientError> {
        ...
        // wire docs.
        let mut url =
            reqwest::Url::parse(&cfg.url).map_err(|e| ChClientError::Other(e.to_string()))?;
        {
...
mod tests {
    ...
    fn leading_write_verb_recognises_common_writes() {
        assert_eq!(
            leading_write_verb("INSERT INTO foo VALUES (1)"),
            Some("INSERT")
        );
        ...
        assert_eq!(
            leading_write_verb("/* x */ TRUNCATE TABLE foo"),
            Some("TRUNCATE")
        );
    }
    ...
    fn leading_write_verb_passes_through_reads() {
        ...
        assert_eq!(
            leading_write_verb("with cte AS (select 1) select * from cte"),
            None
        );
