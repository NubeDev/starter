...
//! `starter-store-warehouse::MigrationRunner::with_extra_migration`
...
use anyhow::Result;
use starter_store_warehouse::{ChClient, ChConfig, MigrationRunner, PgSource};
...
const RUBIX_0002_HISTORY_UP: &str = include_str!("../../migrations/0002_history/up.sql");
...
/// L3 dashboard mart for the data-flow scenario. See
/// `migrations/0005_meter_readings_15m/up.sql` and
/// `rubix/docs/sessions/data-flow/05-dashboard-at-scale.md`.
const RUBIX_0005_METER_READINGS_15M_UP: &str =
    include_str!("../../migrations/0005_meter_readings_15m/up.sql");
...
pub async fn apply_ch_migrations(
    ...
    MigrationRunner::new(&client)
        ...
        .with_extra_migration(
            "rubix/0005_meter_readings_15m/up.sql",
            RUBIX_0005_METER_READINGS_15M_UP,
        )
        .run()
        .await
...
mod tests {
    ...
    fn parse_pg_dsn_extracts_components() {
        let pg =
            parse_pg_dsn("postgres://rubix:rubix-dev@127.0.0.1:5433/rubix").expect("DSN parses");
