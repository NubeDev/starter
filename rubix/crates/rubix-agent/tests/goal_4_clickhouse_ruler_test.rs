...
//!
//! Drives the `rubix.clickhouse.retention.set` write verb through
//! the same [`UndoDispatcher`] seam the agent loop uses, asserts
//! that the underlying `ALTER TABLE … MODIFY TTL` ran (the
//! in-memory [`InMemoryChWriter`] stands in for the production
//! `ChClient`-backed impl), confirms that a snapshot row was
//! recorded in the changelog with `before = ChRetentionSnapshot
//! { days: prior }`, then fires `rubix.undo.last` and asserts the
//! prior TTL is restored.
//!
//! Backing store note: the production CH-backed [`ChWriter`] impl
//! lands in a follow-up phase that wires
//! `starter-store-clickhouse::ChClient` to the same trait. Until
//! then the `InMemoryChWriter` fake is enough — the trait shape is
...
use rubix_spi::dto::clickhouse::retention_set::ClickhouseRetentionSetResponse;
use rubix_tools::clickhouse::retention_set::ClickhouseRetentionSetTool;
use rubix_tools::clickhouse::store::{
    ChRetentionReversible, ChRetentionSnapshot, ChWriter, InMemoryChWriter, CH_RETENTION_KIND,
};
...
async fn retention_set_via_mcp_alters_ttl_records_snapshot_and_undo_restores_prior() {
    ...
    // `ChWriter` trait object.
    let in_mem: Arc<InMemoryChWriter> = Arc::new(InMemoryChWriter::new());
    in_mem.seed_retention("system_disk_history", 90);
    let writer: Arc<dyn ChWriter> = in_mem.clone();

    let reversible = Arc::new(ChRetentionReversible::new(writer.clone()));
    ...
    let retention = UndoDispatcher::new(
        Arc::new(ClickhouseRetentionSetTool::new(writer.clone())),
        registry.clone(),
        recorder.clone(),
        actor_source.clone(),
    );
    ...
    // Assert the underlying ALTER actually ran — InMemoryChWriter
    ...
    let ch = &page.items[0];
    assert_eq!(ch.resource.kind, CH_RETENTION_KIND);
    assert_eq!(ch.resource.id.as_deref(), Some("system_disk_history"));
    assert!(matches!(ch.op, Op::Update));
    let before: ChRetentionSnapshot = serde_json::from_value(
        ch.before.clone().expect("snapshot row has before payload"),
    )
    .expect("before snapshot decodes");
    let after: ChRetentionSnapshot = serde_json::from_value(
        ch.after.clone().expect("snapshot row has after payload"),
    )
