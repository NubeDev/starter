//! `rubix.warehouse.rule.list` — tool dispatch.
//!
//! Read-only: queries `WarehouseWriter::list_rules` and returns the rows
...
/// Concrete [`Tool`] for `rubix.warehouse.rule.list`.
...
mod tests {
    ...
    async fn empty_writer_lists_zero_rules() {
        let tool = WarehouseRuleListTool::new(Arc::new(InMemoryWarehouseWriter::new()));
        let out = tool.invoke(serde_json::json!({})).await.unwrap();
        let resp: WarehouseRuleListResponse = serde_json::from_value(out).unwrap();
        assert_eq!(resp.summary.code.as_str(), "rubix.warehouse.rule.listed");
    ...
    async fn rules_come_back_sorted() {
        ...
        writer
            ...
            .await
            .unwrap();
        let tool = WarehouseRuleListTool::new(writer);
        let out = tool.invoke(serde_json::json!({})).await.unwrap();
        let resp: WarehouseRuleListResponse = serde_json::from_value(out).unwrap();
