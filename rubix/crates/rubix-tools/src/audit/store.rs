...
//! same trait shape, same snapshot-shape Reversible. The trait
//! and row type live in [`rubix_spi::audit`] so this crate and
//! `rubix-store-postgres` share the same contract without
//! depending on each other (SCOPE R5: tools and store-postgres
//! are siblings, both rooted in `rubix-spi`). The production
//! binary swaps in `rubix_store_postgres::PgAuditPolicyStore`
//! without touching the verb files.
//!
...
// Re-export the contract from `rubix-spi::audit` so existing verb
// code (`use crate::audit::store::{AuditPolicyRow, AuditPolicyStore}`)
// keeps compiling after the trait/row moved out of this crate.
pub use rubix_spi::audit::{AuditPolicyRow, AuditPolicyStore, AUDIT_POLICY_KIND};
