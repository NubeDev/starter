//! [`ChWriter`] adapter that targets a live [`ChClient`].
//!
//! The seven `rubix.clickhouse.*` verbs are written against the
//! [`ChWriter`] trait so the production binary can swap a
...
//! [`crate::clickhouse::store::ChWriter`] via a [`ChClient`]
...
use crate::clickhouse::store::{
    ChMartSnapshot, ChRetentionSnapshot, ChRuleSnapshot, ChTableSummary, ChWriter,
};

/// [`ChWriter`] backed by a live [`ChClient`] bound to a named
...
pub struct ChClientWriter {
    client: ChClient,
    database: String,
}

impl std::fmt::Debug for ChClientWriter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChClientWriter")
...
#[async_trait]
impl ChWriter for ChClientWriter {
    ...
    async fn current_retention(&self, table_name: &str) -> Result<Option<u32>> {
        let meta = self.lookup(table_name).await?.ok_or_else(|| Error::NotFound {
    ...
    async fn apply_retention(
        ...
        let prior_meta = self.lookup(table_name).await?.ok_or_else(|| Error::NotFound {
            what: format!("clickhouse table `{table_name}`"),
        })?;
