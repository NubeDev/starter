//! `rubix.clickhouse.tables.list` — request/response DTOs + descriptor.
//!
...
pub struct ClickhouseTablesListRequest {}

/// One table as returned by `rubix.clickhouse.tables.list`.
...
/// Tool reply.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub struct ClickhouseTablesListResponse {
    /// Outcome (`rubix.clickhouse.tables.listed`).
...
pub static DESCRIPTOR: ToolDescriptor = ToolDescriptor {
    ...
    when_to_use: "Use for the warehouse admin overview, or to pick a `table_name` before rubix.clickhouse.retention.set.",
    when_not_to_use: "Do not use to enumerate derived-state rules (rubix.clickhouse.rule.list).",
    example: concat!(
        "Input:  { }\n",
        "Output: { \"summary\": { \"code\": \"rubix.clickhouse.tables.listed\", ",
