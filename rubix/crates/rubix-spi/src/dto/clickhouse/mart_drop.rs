//! `rubix.clickhouse.mart.drop` — request/response DTOs + descriptor.
//!
//! Destructive: removes a mart. The verb itself is a thin shim over
//! `ChWriter::restore_mart` with an empty snapshot (the same code
...
pub struct ClickhouseMartDropRequest {
...
/// Tool reply.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub struct ClickhouseMartDropResponse {
    /// Outcome — `rubix.clickhouse.mart.dropped` on the happy path
    /// or `rubix.clickhouse.mart.absent` when the mart did not
...
pub static DESCRIPTOR: ToolDescriptor = ToolDescriptor {
    ...
    example: concat!(
        ...
        "Output: { \"summary\": { \"code\": \"rubix.clickhouse.mart.dropped\", ",
