//! `rubix.clickhouse.mart.list` — request/response DTOs + descriptor.
//!
//! Read-only sibling of `rubix.clickhouse.mart.create`. Surfaces
//! every mart the backing [`ChWriter`](
//! crate::dto::clickhouse::mart_create::ClickhouseMartCreateRequest)
//! holds.
...
pub struct ClickhouseMartListRequest {}

/// One mart as returned by `rubix.clickhouse.mart.list`.
...
/// Tool reply.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub struct ClickhouseMartListResponse {
    /// Outcome (`rubix.clickhouse.mart.listed`).
...
pub static DESCRIPTOR: ToolDescriptor = ToolDescriptor {
    ...
    when_not_to_use: "Do not use to enumerate derived-state rules (rubix.clickhouse.rule.list) or raw tables (rubix.clickhouse.tables.list).",
    example: concat!(
        "Input:  { }\n",
        "Output: { \"summary\": { \"code\": \"rubix.clickhouse.mart.listed\", ",
