//! `rubix.clickhouse.mart.create` — request/response DTOs and tool descriptor.
//!
...
/// Caller input for `rubix.clickhouse.mart.create`.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub struct ClickhouseMartCreateRequest {
...
/// Tool reply.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub struct ClickhouseMartCreateResponse {
    /// Outcome — `rubix.clickhouse.mart.created` on the happy path
    /// or `rubix.clickhouse.mart.already_exists` when the mart was
...
pub static DESCRIPTOR: ToolDescriptor = ToolDescriptor {
    ...
    when_not_to_use: concat!(
        ...
        "rubix.clickhouse.rule.write with an ALTER statement). Do not ",
        "use to set TTL (rubix.clickhouse.retention.set). Note: undo ",
    ...
    example: concat!(
        ...
        "Output: { \"summary\": { \"code\": \"rubix.clickhouse.mart.created\", ",
    ...
    siblings: &[
        SiblingTool {
            ...
            wins_when: "the target is a derived-state view over an existing mart, not a fresh table.",
        },
        SiblingTool {
            id: "rubix.clickhouse.retention.set",
