//! `rubix.clickhouse.rule.write` — request/response DTOs and tool descriptor.
//!
...
/// Caller input for `rubix.clickhouse.rule.write`.
///
...
pub struct ClickhouseRuleWriteRequest {
...
/// Tool reply.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub struct ClickhouseRuleWriteResponse {
    /// Outcome (`rubix.clickhouse.rule.written`).
...
pub static DESCRIPTOR: ToolDescriptor = ToolDescriptor {
    ...
    when_not_to_use: concat!(
        ...
        "rubix.clickhouse.mart.create). Do not use to change ",
        "retention/TTL (call rubix.clickhouse.retention.set). Never ",
        "use to DROP an object — undo the prior write instead."
    ),
    example: concat!(
        "Input:  { \"rule_name\": \"system_disk_rollup_1h\", ",
        "\"ddl\": \"CREATE MATERIALIZED VIEW ...\" }\n",
        "Output: { \"summary\": { \"code\": \"rubix.clickhouse.rule.written\", ",
