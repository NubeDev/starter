//! `rubix.warehouse.rule.list` — request/response DTOs + descriptor.
//!
//! Read-only sibling of `rubix.warehouse.rule.write`. The backing
//! `WarehouseWriter::list_rules` surfaces every rule the in-memory or
...
pub struct WarehouseRuleListRequest {}

/// One rule as returned by `rubix.warehouse.rule.list`.
...
/// Tool reply.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub struct WarehouseRuleListResponse {
    /// Outcome (`rubix.warehouse.rule.listed`).
...
pub static DESCRIPTOR: ToolDescriptor = ToolDescriptor {
    ...
    when_not_to_use: concat!(
        "Do not use to enumerate marts (use rubix.warehouse.mart.list) ",
        "or raw tables (use rubix.warehouse.tables.list).",
    ),
    example: concat!(
        "Input:  { }\n",
        "Output: { \"summary\": { \"code\": \"rubix.warehouse.rule.listed\", ",
