...
## Status
2026-05-29 — v0 landed end-to-end (primitives + parser + dispatcher integration on both builtin and process dispatchers + canary sidecar + per-spec stats + admin endpoint `GET /api/v1/admin/cache/specs` + 5 scenario tests + 5 integration tests + warehouse-write `// TODO(cache-invalidation):` markers). Numbers still TBD — needs a real workload replay on the rubix dev rig; the admin endpoint is now what to scrape.
...
- **What landed:**
  ...
  - Commit `feat(rubix-agent): GET /api/v1/admin/cache/specs` — read-only admin projection returning `{ "specs": [ { spec_id, hits, misses, hit_ratio } ] }`. Returns `{ "specs": [] }` when the cache layer is not wired (developer rigs). `AdminState` gains `cache_layer` + `with_cache_layer(...)`. Two end-to-end tests in `tests/admin_cache_test.rs`.
- **What's blocked:** Real-workload hit-rate numbers still pending — but now there is a single endpoint to scrape, so any rubix dev rig with the rubixos extension running can answer "is the canary paying off" in one curl.
...
- **Numbers:** Per-spec snapshot endpoint live; hit-rate / latency still pending environment time.
