...
### Phase 2b — starter-mcp + starter-flow-surfaces gaps blocking rubix PR 3 (MCP exposure)

Discovered scoping thin-slice PR 3. PR 3 cannot land without hacks
until the three items below land upstream. Size order (smallest
first); per the Phase 2a one-PR-per-piece discipline, each ships as
its own upstream PR. Sequencing in
[`docs/sessions/2026-05-23-next-steps-6.md`](../../sessions/2026-05-23-next-steps-6.md).

#### U1 — `starter-mcp` Accept-Language plumbing

`starter-mcp` extend · blocks rubix PR 3 · planned, ~150 LOC (smallest).

*Gap.* `starter-mcp::http::handle()` reads only the body; no task-local for session locale (cf. `principal_local`). PR 3's exit signal ("Spanish on `Accept-Language: es-AR`") cannot be met — MCP-dispatched tools cannot see caller language.
...
*Test.* `starter-mcp` round-trip: `tools/call` with `Accept-Language: es-AR`; tool reads `current_locale()` → `es-AR`.

#### U2 — `starter-mcp` `InMemoryTransport` real implementation

`starter-mcp` extend · blocks rubix PR 3 · planned, medium PR.

*Gap.* `InMemoryTransport { _private: () }` is a stub. PR 3 names "round-trip via the MCP testing harness" as the test surface; without one, no end-to-end MCP assertion is possible.
...
*No workaround.* Calling dispatch directly skips serialisation, framing, and `initialize` — the very surface PR 3 must prove. Textbook hack.

*Test.* Round-trip on the new transport: client `tools/list` then `tools/call`; assert framed JSON-RPC.

#### U3 — `starter-flow-surfaces` `FlowRegistry::resolve` + `FlowAsTool::from_registry`

`starter-flow-surfaces` extend · blocks rubix PR 3 · planned, largest.

*Gap.* Wrapping `com.rubix.scheduled-system-check` as a `FlowAsTool` today requires hand-rolled per-flow plumbing: `Engine` + topology + terminal slots + seed/output adapters + explicit schemas (D-F3.4 forbids derivation). `FlowAsTool` docstring (~L76-79) notes auto-resolve "lands later." ~50 lines of glue per flow today.
...
*Test.* `from_registry("com.example.foo", "v1", engine)` returns a `FlowAsTool` whose `dispatch` runs end-to-end and produces a `Diagnostic` matching the topology's terminal slot.
