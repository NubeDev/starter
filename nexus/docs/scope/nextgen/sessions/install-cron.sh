#!/usr/bin/env bash
# Installs (or removes) the cron entry that drives the Nexus build loop every 5 minutes.
...
#
# Kill switch (no crontab edit needed):  touch nexus/docs/scope/nextgen/sessions/.loop.STOP
# Resume:                                rm   nexus/docs/scope/nextgen/sessions/.loop.STOP
set -euo pipefail

TICK="/home/user/code/rust/starter/nexus/docs/scope/nextgen/sessions/loop-tick.sh"
LOG="/home/user/code/rust/starter/nexus/docs/scope/nextgen/sessions/cron.log"
MARKER="# nexus-build-loop"
LINE="*/5 * * * * $TICK $MARKER"
...
case "${1:-install}" in
  ...
  remove)
    ...
    echo "Removed the nexus-build-loop cron entry."
    ;;
  status)
