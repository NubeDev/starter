...
//! an inbound webhook — so it is the baseline channel kind. The only config is
//! the URL; an authenticated webhook (a signing secret / auth header) would store
//! that secret under the R6 envelope, like datasource secrets, when it lands.
