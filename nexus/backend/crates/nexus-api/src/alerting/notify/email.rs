...
//! to the client (the channel read path redacts it). The message is sent as a
//! multipart body (plain text plus a minimal HTML rendering) so it is readable in
//! any client. TLS is via rustls (no native OpenSSL), keeping the build
//! self-contained. A transport or auth failure is returned as a message for the
//! event record and the retry layer — it never panics the evaluator.
...
/// Assemble the RFC 5322 message: a subject derived from the rule + transition,
/// and a multipart body so plain-text-only clients still get the message.
fn build_message(cfg: &EmailConfig, n: &Notification) -> Result<Message, String> {
    let subject = format!("[{}] {}", n.transition, n.rule_name);
...
/// Escape the few characters that would break out of the HTML body. The message
/// is rendered from a closed template token set, but escaping is defence in depth.
...
mod tests {
    ...
    fn note() -> Notification {
        Notification {
            ...
            threshold: 10.0,
            op: "lt".into(),
            message: "Alert Disk is firing".into(),
        }
    }
    ...
    fn message_builds_with_subject_from_transition() {
        ...
        .unwrap();
        let msg = build_message(&cfg, &note());
        assert!(msg.is_ok());
    }
