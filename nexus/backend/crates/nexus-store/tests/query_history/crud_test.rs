...
async fn records_and_lists_newest_first() {
    ...
    query_history::record_run(pg, "acme", &run("SELECT 1")).await.unwrap();
    query_history::record_run(pg, "acme", &run("SELECT 2")).await.unwrap();

    let rows = query_history::list_recent(pg, "acme", "user-1", 10).await.unwrap();
...
async fn starring_pins_a_row_first_and_persists() {
    ...
    query_history::record_run(pg, "acme", &run("SELECT old")).await.unwrap();
    let rows = query_history::list_recent(pg, "acme", "user-1", 10).await.unwrap();
    let old_id = rows[0].id;
    query_history::record_run(pg, "acme", &run("SELECT new")).await.unwrap();

    // Star the older row; it should sort above the newer un-starred one.
    let updated = query_history::set_starred(pg, "acme", "user-1", old_id, true).await.unwrap();
    assert!(updated);
    let rows = query_history::list_recent(pg, "acme", "user-1", 10).await.unwrap();
...
async fn tenant_isolation_holds() {
    ...
    query_history::record_run(pg, "acme", &run("SELECT acme")).await.unwrap();
    query_history::record_run(pg, "other", &run("SELECT other")).await.unwrap();

    let acme = query_history::list_recent(pg, "acme", "user-1", 10).await.unwrap();
...
async fn starring_another_users_row_is_a_no_op() {
    ...
    query_history::record_run(pg, "acme", &run("SELECT mine")).await.unwrap();
    let rows = query_history::list_recent(pg, "acme", "user-1", 10).await.unwrap();
    let id = rows[0].id;

    // A different user cannot star user-1's row.
    let updated = query_history::set_starred(pg, "acme", "user-2", id, true).await.unwrap();
    assert!(!updated);
}
