...
async fn set_upserts_and_replaces_the_whole_bag() {
    ...
    user_settings::set(pg, "acme", "user-1", &first).await.unwrap();
    assert_eq!(user_settings::get(pg, "acme", "user-1").await.unwrap(), first);
    ...
    user_settings::set(pg, "acme", "user-1", &second).await.unwrap();
    assert_eq!(user_settings::get(pg, "acme", "user-1").await.unwrap(), second);
}
...
async fn settings_are_isolated_per_user_and_tenant() {
    ...
    user_settings::set(pg, "acme", "user-1", &mine).await.unwrap();
