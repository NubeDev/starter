...
pub async fn insert(
    ...
    let row = sqlx::query(
        "INSERT INTO nexus_notify_silences (tenant_id, detection_id, starts_at, ends_at, reason, created_by) \
...
pub async fn list(pool: &PgPool, tenant_id: &str) -> Result<Vec<SilenceRecord>, Error> {
    let mut tx = tenant_tx::begin(pool, tenant_id).await?;
    let rows = sqlx::query(
        "SELECT id, detection_id, starts_at, ends_at, reason FROM nexus_notify_silences ORDER BY created_at DESC",
    )
    .fetch_all(&mut *tx)
    .await
    .map_err(internal)?;
...
/// Whether an active silence currently covers `detection_id` — either
/// detection-specific or tenant-wide (`detection_id IS NULL`) — at `now`. The
/// runner checks this before notifying.
pub async fn is_silenced(
    ...
    let mut tx = tenant_tx::begin(pool, tenant_id).await?;
    let row = sqlx::query(
        "SELECT 1 FROM nexus_notify_silences \
         WHERE (detection_id = $1 OR detection_id IS NULL) AND starts_at <= $2 AND ends_at > $2 LIMIT 1",
    )
    .bind(detection_id)
    .bind(now)
...
pub async fn delete(pool: &PgPool, tenant_id: &str, id: Uuid) -> Result<bool, Error> {
    let mut tx = tenant_tx::begin(pool, tenant_id).await?;
    let done = sqlx::query("DELETE FROM nexus_notify_silences WHERE id = $1")
        .bind(id)
        .execute(&mut *tx)
        .await
        .map_err(internal)?;
...
fn row_to_silence(row: &sqlx::postgres::PgRow) -> SilenceRecord {
    SilenceRecord {
        id: row.get::<Uuid, _>("id"),
        detection_id: row.get::<Option<Uuid>, _>("detection_id"),
