...
pub async fn insert(
    ...
    let row = sqlx::query(
        "INSERT INTO nexus_alert_channels (tenant_id, name, kind, config) \
...
pub async fn list(pool: &PgPool, tenant_id: &str) -> Result<Vec<ChannelRecord>, Error> {
    let mut tx = tenant_tx::begin(pool, tenant_id).await?;
    let rows = sqlx::query(
        "SELECT id, tenant_id, name, kind, config FROM nexus_alert_channels ORDER BY created_at DESC",
    )
    .fetch_all(&mut *tx)
    .await
    .map_err(internal)?;
...
/// Fetch the channels with the given ids within the tenant — the evaluator's
/// lookup when fanning a transition out to a rule's targets.
pub async fn by_ids(
    pool: &PgPool,
    tenant_id: &str,
    ids: &[Uuid],
    ...
    let mut tx = tenant_tx::begin(pool, tenant_id).await?;
    let rows = sqlx::query(
        "SELECT id, tenant_id, name, kind, config FROM nexus_alert_channels WHERE id = ANY($1)",
    )
    .bind(ids)
    .fetch_all(&mut *tx)
    .await
...
pub async fn delete(pool: &PgPool, tenant_id: &str, id: Uuid) -> Result<bool, Error> {
    let mut tx = tenant_tx::begin(pool, tenant_id).await?;
    let done = sqlx::query("DELETE FROM nexus_alert_channels WHERE id = $1")
        .bind(id)
        .execute(&mut *tx)
        .await
        .map_err(internal)?;
    ...
    if let sqlx::Error::Database(db) = &e {
        if db.is_unique_violation() {
            return Error::Conflict {
                message: "an alert channel with that name already exists".into(),
            };
        }
    }
    internal(e)
